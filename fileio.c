...
void delFile(char path) {
	...
	readSector(mapBuffer, 256);
	readSector(folderAndFiles, 257);
	readSector(folderAndFiles + 512, 258);
	readSector(sectBuffer, 259);
 
    i = 0;
    while (sectBuffer[folderAndFiles[path * 16 + 1] * 16 + i] != '\0' && i < 16) {
        // Kosongkan
        mapBuffer[sectBuffer[folderAndFiles[path * 16 + 1] * 16 + i]] = 0x0;
        sectBuffer[folderAndFiles[path * 16 + 1] * 16 + i] = 0x0;
        i++;
    }
	...
	writeSector(sectBuffer, 259);
	writeSector(mapBuffer, 256);
}